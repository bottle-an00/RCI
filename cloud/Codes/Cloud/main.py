...
#
# 새로 받은 아이콘 중 진단·강제구동은 그림 안의 자동차가 주인공이다. 대상이 UR 로봇인
# 화면에 그대로 쓰면 '지금 무엇을 다루는 중인가' 를 잘못 알려주므로 UR 에서는 대상을
# 가리지 않는 기존 SVG 로 되돌린다.
#
# 실습 준비(prep-car)는 여기서 뺐다 — 그림의 주인공이 '체크리스트 문서'고 차는 곁에
# 붙은 소품이다. 준비 단계에서 하는 일(읽고 확인하기)은 대상과 무관해 UR 에서도 같은
# 그림을 쓰는 편이 두 대상의 화면을 같은 것으로 읽게 한다. 이론 교육(문서)·ECU
# 업그레이드(화살표)도 같은 이유로 대상을 가리지 않는다.
_CAR_ICON_ALT = {"diag-car": "sensor", "force-car": "robotarm"}
...
def content_icon(content, target):
    """컨텐츠 아이콘 이름. 대상이 UR 로봇이면 자동차가 주인공인 그림을 피해 간다.

    새로 받은 그림 아이콘 중 진단·강제구동은 자동차가 그림의 주인공이다. UR 로봇
    화면에 그대로 걸면 '지금 무엇을 다루는 중인가' 를 잘못 알려주므로, 대상을 가리지
    않는 기존 SVG 로 되돌린다(_CAR_ICON_ALT). 실습 준비·이론 교육·ECU 업그레이드는
    대상이 드러나지 않아 두 대상이 같은 그림을 쓴다.
    """
    name = content["icon"]
