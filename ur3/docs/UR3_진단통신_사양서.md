...
세션을 더 유지할 필요가 없어진 시점(사용자가 진단 화면을 벗어남, 명시적으로 `10 01` 기본세션 복귀를 요청함 등)에는 주기 발행을 멈춘다. 계속 보내도 RCI가 거부하진 않지만(무의미한 트래픽일 뿐), 웹앱 쪽에서 불필요한 백그라운드 작업을 정리하는 차원의 규칙이다.
...
| 0x0305 | Wave (인사 동작) | 속도(1B,%)+가속도(1B,%)+반복횟수(1B,1~10) | 확장 세션 + 보안 접근 완료 |
| 0x0306 | TableTour (모서리 순회+그리퍼 시연) | 속도(1B,%)+가속도(1B,%)+그리퍼반복횟수(1B,1~10) | 확장 세션 + 보안 접근 완료 |

고정 인사 자세로 이동 → 손목(wrist2/wrist3) 좌우 왕복 → 원래 위치로 복귀까지 한
번의 요청으로 전부 처리하는 시연용 동작이다. 베이스/숄더/엘보는 왕복 구간에서
고정한 채 손목만 크게 움직인다. 별도로 `MoveJoint`를 먼저 보낼 필요가 없다.

TableTour(0x0306)는 테이블 4모서리(고정 좌표, 가정치)를 순서대로 찍고 중앙으로
이동한 뒤, 그리퍼 개폐를 지정한 횟수만큼 반복하고 원래 위치로 복귀한다. 그리퍼는
실제 I/O가 아직 미확정이라 상태값(`last_gripper_cmd`)만 토글되고 로그로 남는다
(§6.7 IOCBI 0x0206 그리퍼와 동일한 미확정 상태를 공유).

```
요청 : 31 01 03 01 00 00 FC 7C 00 00 FC 7C 00 00 00 00 1E 1E   # MoveJoint 시작(별개 예시)
응답 : 71 01 03 01 00                                           # 즉시 접수

요청 : 31 01 03 05 32 32 03                                     # Wave 시작(속도50%,가속도50%,3회) — 단독 호출로 충분
응답 : 71 01 03 05 00                                           # 즉시 접수
